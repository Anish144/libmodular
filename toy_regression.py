...
def create_sparse_summary(sparse_ops):
    def layer_sparsity(op):
        batch_sparse = tf.reduce_sum(
            op, axis=1)/tf.cast((tf.shape(op)[1]), tf.float32)
...
if __name__ == '__main__':
    ...
    m_step, eval = modular.modularize_variational(
        ...
        sample_size, iteration_number, epoch_lim, damp_length,
        alpha)
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter(
            f'toy/TOY_REgression_With_batch_norm_MOD_{time}')
        ...
        target_1 = x_1 @ np.array([[100, 0], [0, 50]])
        theta = 85
        target_2 = rotate((0, 0), x_2, math.radians(theta))
