...
def run():
    ...
    def network(context: modular.ModularContext, masked_bernoulli=False, variational=variational):
        ...
        if masked_bernoulli:
            ...
            modules = modular.create_dense_modules(inputs, module_count, units=128, activation=tf.nn.relu) 
            hidden, l1, _ = modular.variational_mask(inputs, modules, context, 0.001, 3.17) #[sample * B x units]

            # modules = modular.create_dense_modules(hidden, module_count, units=64, activation=tf.nn.relu) 
            # hidden, l2, _ = modular.variational_mask(hidden, modules, context, 0.001, 3.17) #[sample * B x units]

            # modules = modular.create_dense_modules(hidden, module_count, units=32, activation=tf.nn.relu) 
            # hidden, l3, _ = modular.variational_mask(hidden, modules, context, 0.001, 3.17) #[sample * B x units]

            modules = modular.create_dense_modules(hidden, module_count, units=10) 
            logits, l4, bs = modular.variational_mask(hidden, modules, context, 0.001, 3.17) #[sample * B x units]
            ...
            # modules = modular.create_dense_modules(hidden, module_count, units=64, activation=tf.nn.relu) 
            # hidden, l2, _ = modular.new_controller(hidden, modules, context,  get_initialiser(dataset_size, 5, module_count)) #[sample * B x units]

            # modules = modular.create_dense_modules(hidden, module_count, units=32, activation=tf.nn.relu) 
            # hidden, l3, _ = modular.new_controller(hidden, modules, context,  get_initialiser(dataset_size, 5, module_count)) #[sample * B x units]

            modules = modular.create_dense_modules(hidden, module_count, units=10) 
            logits, l2, bs = modular.new_controller(hidden, modules, context,  get_initialiser(dataset_size, 5, module_count)) #[sample * B x units]
        ...
        return loglikelihood, logits, accuracy, selection_entropy, batch_selection_entropy, bs_1, l2, l1, bs
    ...
    ll, logits, accuracy, s_entropy, bs_entropy, bs_1, l2, l1, bs = eval
    ...
    try:
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter(f'logs/train:_15m_new_controller_KL:True_{time}',sess.graph)
            test_writer = tf.summary.FileWriter(f'logs/test:_15m_new_controller_KL:True_{time}',sess.graph)
