...
def modular_layer(
    inputs, modules: ModulePool, parallel_count: int, context: ModularContext
):
    """
    ...
    with tf.variable_scope(None, 'modular_layer'):
        ...
        best_selection_persistent = tf.get_variable(
            'best_selection', shape, tf.int32, initializer)
        # Different for each layer

        if context.mode == ModularMode.E_STEP:
            # Use gather to get the selection of
            # the batch indices of the data [1...32] then [32...64]
            # [1 x Batch x parallel]
            best_selection = tf.gather(
                best_selection_persistent,
                context.data_indices)[tf.newaxis]  # [1 x B x parallel]
            samples = ctrl.sample()
        ...
        return (run_masked_modules_withloop_and_concat(
            inputs, selection,
            modules.module_fnc, modules.output_shape),
            logits, best_selection_persistent)


def masked_layer(
    inputs, modules: ModulePool, context: ModularContext, initializer
):
    """
    Function that takes in input and modules and return the selection based
    on a Bernoulli mask
    Args:
    ...
    with tf.variable_scope(None, 'masked_layer'):
        ...
        ctrl_bern = tfd.Bernoulli(
            logits=logits)  # Create controller with logits
        ...
        best_selection_persistent = tf.get_variable(
            'best_selection',
            shape=shape,
            dtype=tf.int32, initializer=initializer)
        # Different for each layer
...
def variational_mask(
    ...
    with tf.variable_scope(None, 'variational_mask'):
        ...
        tau = 0.01
        z = relaxed_bern(
            tau, pi_batch, [tile_shape * context.sample_size, shape])
        ...
        if inputs.shape.ndims > 3:
            ...
            new_weights = tf.einsum(
                'mio,bm->bmio', modules.weight, tf.cast(z, tf.float32))
            new_biases = tf.einsum(
                'mo,bm->bmo', modules.bias, tf.cast(z, tf.float32))
